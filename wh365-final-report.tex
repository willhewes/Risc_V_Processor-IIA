...
This learning phase was followed by a series of small experiments within the Verilog files, 
which helped build confidence with the structure of the codebase. 
In hindsight, some of these early edits were made without a broader strategy in place, 
...
which later included stricter version control, more careful integration with other modules, 
and the use of targeted testbenches and synthesis diffs to catch problems earlier.

As a team we worked together through git, allowing us to collaborate effectively.
We set up different branches from the main for each of us to use, 
with the plan being that we would work on some changes independently 
and then merge our changes cohesively.
My workflow initially consisted of running the \textit{make, make install} etc.
...
I frequently relied on a block of code pasted at once 
